...
def parse_test_cases( path, perform, base_path='.', optional_out=False, include_path=None):
...
class TestItem:
    ...
    id = None
    arg = None
    data = None # Text data, array of 8-bit strings
    file = None
    line = None # Line number in file
    
    def __init__(self, id, arg, data, file, line):
        self.id = id
        self.arg = arg
        self.data = data
        self.file = file
...
def collapse_line_continuation(l):
    r = []
    cont = False
    for s in l:
        ss = re.sub(r'\\$', '', s)
...
def expand_includes(a, base_path):
